...
| **Square corners** | ~130 files. Radius appears in FOUR forms, all zeroed (except ≥100 = true circles): (1) `Radius.circular(N)` calls, (2) `static const ... radius = N`, (3) inline `radius: N` / `borderRadius: N` args, (4) **default-valued constructor params** `this.borderRadius = 20` on the core widgets (`tmail_toast`, `tmail_button_widget`, `tmail_container_widget`, `material_text_button/icon_button`, `highlight_svg_icon_on_hover`, `side_arrow_clipper`). Form (4) is the one that silently re-rounds everything — check it first. | Upstream adds rounded widgets over time; re-run all four sweeps if new rounded corners appear. |
| **System messages** | `core/lib/presentation/extensions/color_extension.dart` (toast bg: success `#2E7D32`, error `#AC0014`, warning `#B7791F`; `colorBackgroundNotificationVacationSetting`) | Toasts/banners themed to Scopos anchors; text is Barlow via fontApp. |
